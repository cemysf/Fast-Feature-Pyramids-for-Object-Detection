...
int main()
{
	...
	fp_pos = fopen(filename, "r");  //fopen(&fp_pos, filename, "r");
	...
	sprintf(filename, "neg_bagging/%d.txt", i+1);

	fp_neg = fopen(filename,"r");	//fopen(&fp_neg, filename, "r");
	...
	*/
	...
	for (int i = 0; i < 288; i++)
	{
		...
		fp_groundtruth = fopen(filename_groundtruth, "r"); 	//fopen(&fp_groundtruth, filename_groundtruth, "r");
	...
	FILE* result;
	result = fopen("result.txt","w");	//fopen(&result, "result.txt", "w");
	for (int p = 0; p < 100; p++)
